# Sprint 01 — Base do portfólio

**Estado:** artefatos iniciais preparados.
**Integrantes:** Gabriel Nottoli Buck (RA 10425384) e Julia Andrade (RA 10427828).

## Objetivo

Estabelecer problema, recorte e documentação rastreável do BookLog.

## Entregas

- [x] README e estrutura documental.
- [x] Proposta de visão, valor e personas.
- [x] RF, RNF, regras, stakeholders e glossário.
- [x] Casos de uso, wireframes e modelos iniciais.
- [x] Arquitetura proposta e pipeline documental.

As marcações concluídas indicam arquivos criados, não aprovação nem software implementado.

---

[Índice da documentação](../README.md) · [Página do projeto](../../README.md)
